...
def _set_table_widths(table, keys, rows, landscape=False):
    """Set widths defensively, including one-column tables.

    The previous implementation always created two width ratios when there
    were two or fewer columns. A one-column table therefore had a ratio for a
    nonexistent second cell and raised IndexError during Word generation.
    """
    ...
    if column_count == 1:
        ...
        total = sum(scores) or column_count
        ratios = [x / total for x in scores]

    # Never index beyond the actual cell count. This also makes the helper
    # tolerant of future Word table transformations such as merged cells.
    for row in table.rows:
        cell_count = len(row.cells)
        limit = min(column_count, len(ratios), cell_count)
        for i in range(limit):
            row.cells[i].width = Cm(max(1.25, available * ratios[i]))
...
def add_table(document, data, title, caption, max_rows=300):
    ...
    table = document.add_table(rows=1, cols=len(keys)); table.style = "Table Grid"; table.alignment = WD_TABLE_ALIGNMENT.CENTER; table.autofit = False; _borders(table)
...
def heatmap_figure(case01,case02,case03):
    ...
    fig,ax=plt.subplots(figsize=(9,6.8)); ax.set_xlim(.5,5.5); ax.set_ylim(.5,5.5); ax.set_xticks(range(1,6)); ax.set_yticks(range(1,6)); ax.set_xlabel("Tác động"); ax.set_ylabel("Xác suất"); ax.set_title("Ma trận rủi ro tích hợp",fontsize=14,fontweight="bold"); ax.grid(True,linewidth=.5)
...
def roadmap_figure():
    fig,ax=plt.subplots(figsize=(11,3.2)); ax.axis("off"); stages=[("Kiểm chứng kỹ thuật","Kiểm chứng dữ liệu và kiến trúc"),("Thử nghiệm giới hạn","Giao dịch và kiểm soát giới hạn"),("Triển khai chính thức","Mở rộng và vận hành")]; xs=[.17,.5,.83]
    for i,((name,desc),x) in enumerate(zip(stages,xs)):
        ax.text(x,.58,name,ha="center",va="center",fontsize=12,fontweight="bold",bbox=dict(boxstyle="round,pad=.6",facecolor="white",edgecolor="#667085")); ax.text(x,.27,desc,ha="center",va="center",fontsize=9)
        if i<2: ax.annotate("",xy=(xs[i+1]-.08,.58),xytext=(x+.08,.58),arrowprops=dict(arrowstyle="->",lw=1.3,color="#667085"))
    fig.tight_layout(); out=BytesIO(); fig.savefig(out,format="png",dpi=190,bbox_inches="tight",facecolor="white"); plt.close(fig); out.seek(0); return out
...
def build_docx(profile,financials,case01,case02,case03,report,consistency_results,quality_checks=None):
    ...
    p=document.add_paragraph(); p.alignment=WD_ALIGN_PARAGRAPH.CENTER; p.paragraph_format.space_before=Pt(75); r=p.add_run("ĐỀ ÁN XÂY DỰNG HỆ SINH THÁI BLOCKCHAIN\nTÍCH HỢP TÍN DỤNG, HUY ĐỘNG VỐN VÀ ĐẦU TƯ"); r.bold=True; r.font.name=FONT_NAME; r.font.size=Pt(20)
    ...
    document.add_page_break(); document.add_heading("Phần 1. Hồ sơ case cá nhân",level=1); document.add_paragraph(report.get("part1","").strip() or "Chưa có nội dung phân tích của sinh viên."); profile_rows=[{"Thông số":"MSSV","Giá trị":profile.get("student_id","")},{"Thông số":"D1","Giá trị":profile.get("D1","")},{"Thông số":"D2","Giá trị":profile.get("D2","")},{"Thông số":"D3","Giá trị":profile.get("D3","")},{"Thông số":"D4","Giá trị":profile.get("D4","")},{"Thông số":"Ngành hoạt động","Giá trị":profile.get("industry","")},{"Thông số":"Loại hình doanh nghiệp","Giá trị":profile.get("business_type","")},{"Thông số":"Vấn đề ngân hàng trọng tâm","Giá trị":profile.get("banking_problem","")},{"Thông số":"Công cụ huy động vốn Case 03","Giá trị":profile.get("funding_instrument","")},{"Thông số":"Mô tả hoạt động","Giá trị":profile.get("business_description","")}]; add_table(document,profile_rows,"Hồ sơ doanh nghiệp","Bảng 1.1. Hồ sơ case cá nhân"); add_table(document,_financial_summary(financials),"Cấu trúc vốn","Bảng 1.2. Cấu trúc vốn của case")
    as_is=sorted(case01.get("as_is",[]),key=lambda x:int(x.get("Thứ tự",x.get("Bước",0)) or 0)); document.add_page_break(); document.add_heading("Phần 2. Phân tích vấn đề và quy trình hiện tại",level=1); document.add_paragraph(report.get("part2","").strip() or "Chưa có nội dung phân tích của sinh viên."); _add_figure(document,flow_figure("Quy trình As Is",[x.get("Hành động","") for x in as_is]),"Hình 2.1. Quy trình As Is"); add_table(document,as_is,"Quy trình As Is","Bảng 2.1. Quy trình As Is"); seen=set(); stakeholders=[]
    for row in as_is:
    ...
    document.add_page_break(); document.add_heading("Phần 6. Quản trị rủi ro tích hợp",level=1); document.add_paragraph(report.get("part6","").strip() or "Chưa có nội dung phân tích của sinh viên."); _add_figure(document,heatmap_figure(case01,case02,case03),"Hình 6.1. Ma trận rủi ro tích hợp"); combined=[]
    ...
    document.add_page_break(); document.add_heading("Phần 8. Lộ trình triển khai",level=1); document.add_paragraph(report.get("part8","").strip() or "Chưa có nội dung phân tích của sinh viên."); _add_figure(document,roadmap_figure(),"Hình 8.1. Lộ trình triển khai"); add_table(document,[{"Giai đoạn":"Kiểm chứng kỹ thuật","Mục tiêu":"Kiểm chứng kỹ thuật","Phạm vi":"Dữ liệu và giao dịch mô phỏng","Điều kiện chuyển":"Kiểm chứng được kiến trúc và kiểm soát rủi ro"},{"Giai đoạn":"Thử nghiệm giới hạn","Mục tiêu":"Kiểm chứng vận hành","Phạm vi":"Giao dịch giới hạn","Điều kiện chuyển":"Đạt KPI và yêu cầu pháp lý"},{"Giai đoạn":"Triển khai chính thức","Mục tiêu":"Mở rộng vận hành","Phạm vi":"Hệ sinh thái đầy đủ","Điều kiện chuyển":"Phê duyệt quản trị và sẵn sàng vận hành"}],"Lộ trình triển khai","Bảng 8.1. Lộ trình triển khai")
