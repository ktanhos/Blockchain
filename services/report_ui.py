...
def render_report_builder(st_module, project_id, student_id, profile, financials, case01, case02, case03):
    ...
    st.caption("Xây dựng báo cáo tổng hợp theo đúng cấu trúc trong Instruction File. Case 01, Case 02 và Case 03 được tự động liên kết; sinh viên tập trung viết phân tích và nhận xét.")
    ...
    with right:
        ...
        with st.expander("Dữ liệu tự động từ Case", expanded=True):
            ...
            if sid == "part4":
                st.write(f"DSCR: {case02.get('DSCR', financials.get('DSCR', ''))}")
                st.write(f"LTV: {case02.get('LTV', financials.get('LTV', ''))}")
            if sid == "part5":
