...
def default_case03(profile: dict, financials: dict) -> dict:
    return {
        ...
        "collateral_shared": False, "priority_rights": "Không dùng chung tài sản bảo đảm giữa khoản vay FutureBank và token trong cấu trúc cơ sở.", "smart_contract": "IF Investor_KYC_Status != Valid THEN Reject_Subscription; IF Investor_Exceeds_Ownership_Limit THEN Reject_Transfer; IF Funds_Received == TRUE THEN Issue_Token; IF Minimum_Funding_Target_Not_Reached THEN Refund_Investors; IF Project_Milestone_Verified == TRUE THEN Release_Next_Tranche; IF Payment_Date_Reached AND Issuer_Has_Sufficient_Funds THEN Distribute_Income; IF Security_Incident == TRUE THEN Pause_Transfer", "investor_return": False, "scenarios": ["Cơ sở", "Tăng trưởng", "Suy giảm"],
        ...
        "recommendation": "Chưa kết luận. Đánh giá dòng tiền, tín dụng, pháp lý, thanh khoản và quản trị trước khi phát hành.", "conclusion": "Chưa kết luận.",
    }
