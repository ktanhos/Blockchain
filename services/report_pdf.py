...
def _table(rows, font, widths=None, small=False):
    ...
    if widths is None and converted and converted[0]:
        widths = [180 * mm / len(converted[0])] * len(converted[0])
...
def _records_table(records, font, max_rows=100):
    ...
    if isinstance(records, dict):
        ...
        records = [{"Nội dung": records}]
    elif not isinstance(records, (list, tuple)):
        records = [{"Nội dung": str(records)}]
    records = [r if isinstance(r, dict) else {"Nội dung": str(r)} for r in records]
    keys = []
    for row in records:
        for k in row.keys():
            if k not in keys:
...
def build_integrated_report(profile, financials, case01, case02, case03, consistency_results):
    ...
    story = [Spacer(1, 25 * mm), _paragraph("ĐỀ ÁN XÂY DỰNG HỆ SINH THÁI BLOCKCHAIN TÍCH HỢP TÍN DỤNG, HUY ĐỘNG VỐN VÀ ĐẦU TƯ", title), Spacer(1, 8 * mm), _paragraph("Vai trò của FutureBank", title), Spacer(1, 12 * mm)]
    ...
    story.extend(_paragraph(x, body) for x in executive)
    ...
    story += [Spacer(1, 5 * mm), _table(financial_rows, font, widths=[70 * mm, 110 * mm])]

    story += [_paragraph("2. Case 01 · Phân tích và kiến trúc Blockchain", h1), _paragraph("As Is Process", h2), _records_table(case01.get("as_is", []), font), _paragraph("Đánh giá cơ sở dữ liệu và Blockchain/DLT", h2), _records_table(case01.get("assessment", []), font), _paragraph("Kiến trúc và quản trị", h2)]
    ...
    story += [_table(arch_rows, font, widths=[60 * mm, 120 * mm]), _paragraph("Ma trận quyền", h2), _records_table(case01.get("permissions", []), font), _paragraph("On Chain và Off Chain", h2), _records_table(case01.get("data", []), font), _paragraph("Risk Register", h2), _records_table(case01.get("risks", []), font), _paragraph("Kết luận Case 01", h2), _paragraph(case01.get("conclusion", "Chưa nhập"), body)]

    story += [_paragraph("3. Case 02 · Sản phẩm tín dụng", h1), _paragraph("Các chỉ tiêu tài chính", h2)]
    ...
    story += [_table(case02_rows, font, widths=[65 * mm, 115 * mm]), _paragraph("To Be Process", h2), _records_table(case02.get("to_be", []), font), _paragraph("Kịch bản", h2), _records_table(case02.get("scenarios", []), font), _paragraph("Risk Register", h2), _records_table(case02.get("risks", []), font), _paragraph("Kết luận Case 02", h2), _paragraph(case02.get("conclusion", "Chưa nhập"), body)]
    ...
    story += [_table(status_rows, font, small=True), Spacer(1, 4 * mm)]
    ...
    story += [_paragraph("6. Kết luận chung", h1), _paragraph("Kết luận cuối cùng cần phản ánh đồng thời kết quả của Case 01, Case 02 và Case 03, đồng thời không được bỏ qua các điểm chưa nhất quán được Consistency Checker phát hiện.", body), _paragraph("Báo cáo này là bản tổng hợp dữ liệu đã nhập trong ứng dụng. Sinh viên cần rà soát nội dung, bổ sung phân tích, trích dẫn và tài liệu tham khảo theo yêu cầu của học phần trước khi nộp.", note)]
