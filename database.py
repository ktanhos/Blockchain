...
def save_project(project_id: str, student_id: str, profile: dict, case01: dict, case02: dict | None = None, case03: dict | None = None):
    conn = get_connection()
    existing = conn.execute("SELECT case02_json, case03_json FROM projects WHERE project_id = ?", (project_id,)).fetchone()
    ...
    conn.execute("""
        INSERT INTO projects(project_id, student_id, profile_json, case01_json, case02_json, case03_json)
        VALUES (?, ?, ?, ?, ?, ?)
    ...
    """, (project_id, student_id, json.dumps(profile, ensure_ascii=False), json.dumps(case01, ensure_ascii=False), json.dumps(case02, ensure_ascii=False), json.dumps(case03, ensure_ascii=False)))
    conn.commit()
    conn.close()
...
def load_project(project_id: str):
    conn = get_connection()
    row = conn.execute("SELECT student_id, profile_json, case01_json, case02_json, case03_json FROM projects WHERE project_id = ?", (project_id,)).fetchone()
    conn.close()
    if not row:
        return None
    return {
